...
import { Loader2, Upload, Copy, Twitter, Instagram, Linkedin, Clock, Zap, Sparkles } from "lucide-react";
import { GoogleGenerativeAI, Part } from "@google/generative-ai";
import ReactMarkdown from "react-markdown";
import { Navbar } from "@/components/Navbar";
import { SignInButton, useUser } from "@clerk/nextjs";
import { useRouter } from "next/navigation";
import { getUserPoints, saveGeneratedContent, updateUserPoints, getGeneratedContentHistory, createOrUpdateUser } from "@/utils/db/actions";
...
export default function GenerateContent() {
  ...
  const fetchUserPoints = async () => {
    if (user?.id) {
      ...
      const points = await getUserPoints(parseInt(user.id));
  ...
  const fetchContentHistory = async () => {
    if (user?.id) {
      const contentHistory = await getGeneratedContentHistory(parseInt(user.id));
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // Update points
      const updatedUser = await updateUserPoints(parseInt(user.id), -POINTS_PER_GENERATION);
      if (updatedUser) {
        setUserPoints(updatedUser.points);
      }

      // Save generated content
      const savedContent = await saveGeneratedContent(parseInt(user.id), content.join("\n\n"), prompt, contentType);
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-gray-900 to-black">
        <motion.div
          ...
          className="text-center bg-gray-800 p-8 rounded-2xl shadow-lg max-w-md w-full mx-4"
        >
          <Sparkles className="w-16 h-16 text-blue-400 mx-auto mb-6" />
          <h1 className="text-3xl font-bold text-white mb-4">Welcome to ThreadCraft AI</h1>
          <p className="text-gray-300 mb-8">To start generating amazing content, please sign in or create an account.</p>
          <SignInButton mode="modal">
            <Button className="bg-blue-600 hover:bg-blue-700 text-white px-8 py-3 rounded-xl text-lg font-medium transition-all duration-300 transform hover:scale-105">
              Sign In / Sign Up
            </Button>
          </SignInButton>
        ...
        <div className="grid grid-cols-1 mt-14 lg:grid-cols-3 gap-8">
            ...
            <AnimatePresence>
              {history.map((item) => (
                <motion.div
                  ...
                  className="p-4 bg-gray-700 rounded-xl hover:bg-gray-600 transition-all duration-300 cursor-pointer mb-4"
                  onClick={() => handleHistoryItemClick(item)}
                >
                  <div className="flex items-center mb-2">
                    {item.contentType === "twitter" && <Twitter className="mr-2 h-5 w-5 text-blue-400" />}
                    {item.contentType === "instagram" && <Instagram className="mr-2 h-5 w-5 text-pink-400" />}
                    {item.contentType === "linkedin" && <Linkedin className="mr-2 h-5 w-5 text-blue-600" />}
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.5 }}
              className="bg-gray-800 p-6 rounded-2xl flex items-center justify-between"
            >
              ...
              <Button className="bg-blue-600 hover:bg-blue-700 text-white text-sm py-2 px-6 rounded-xl transition-all duration-300 transform hover:scale-105">
            ...
            >
                ...
                <div>
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <label
                      ...
                      className="cursor-pointer flex items-center justify-center px-4 py-2 bg-gray-700 rounded-xl text-sm font-medium hover:bg-gray-600 transition-all duration-300"
                    >
              ...
              <Button
                ...
                className="w-full bg-blue-600 hover:bg-blue-700 text-white py-3 rounded-xl transition-all duration-300 transform hover:scale-105"
              >
                {isLoading ? (
                  <>
                          ...
                          >
                            ...
                            <div className="flex justify-between items-center text-gray-400 text-xs mt-2">
                              ...
                              <Button
                                ...
                                className="bg-gray-600 hover:bg-gray-500 text-white rounded-full p-2 transition-all duration-300"
                              >
            ...
            <AnimatePresence>
              {generatedContent.length > 0 && (
                ...
                >
                  <h2 className="text-2xl font-semibold mb-4 text-blue-400">Preview</h2>
                  {renderContentMock()}
                </motion.div>
              )}
            </AnimatePresence>
